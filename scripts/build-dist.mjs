...
export function deploy() {
  ...
  const seedDefaults = {
    ...
    HERMES_CLIENT_BOOTSTRAP_EMAIL: process.env.HERMES_CLIENT_BOOTSTRAP_EMAIL || 'admin@local.hermes',
    HERMES_CLIENT_BOOTSTRAP_PASSWORD:
  ...
  if (!existsSync(envDist)) {
    ...
    const updated = lines.map((line) => {
      ...
      if (eq > 0 && !line.trim().startsWith('#')) existingKeys.add(line.slice(0, eq).trim());
    ...
    for (const [k, v] of Object.entries(seedDefaults)) {
      if (!existingKeys.has(k) && !seen.has(k)) updated.push(`${k}=${v}`);
    }
