...
const SOURCE_LABEL: Record<SkillSource, TranslationKey | null> = {
  hub: null,
...
export default function SkillRow({ skill }: { skill: SkillInfo }) {
  ...
  const sourceKey = SOURCE_LABEL[skill.source];
  ...
  return (
    ...
    >
      ...
      {showTrust && (
        <Chip
          label={t('skills.trust', { trust: skill.trust })}
          size="small"
          variant="outlined"
          sx={{
            height: 20,
      ...
      <Chip
        label={sourceKey ? t(sourceKey) : skill.source}
        size="small"
