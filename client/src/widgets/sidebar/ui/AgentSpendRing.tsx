...
const AgentSpendRing = memo(function AgentSpendRing({
  ...
  const rows: Row[] = row
    ? [
        buildRow(t('sidebar.daily'), row.spendWindows.dayUsd, row.caps.dailyCapUsd),
        buildRow(t('sidebar.monthly'), row.spendWindows.monthUsd, row.caps.monthlyCapUsd),
        buildRow(t('sidebar.allTime'), row.spendWindows.allTimeUsd, row.caps.allTimeCapUsd),
      ]
    : [];
    ...
    <Box sx={{ minWidth: 220, py: 0.25 }}>
      ...
      {(modelLabel || !model) && (
        ...
        >
          ...
          >
            {t('sidebar.profile', { profile: hermesProfile })}
          </Typography>
        </Box>
      )}
    </Box>
