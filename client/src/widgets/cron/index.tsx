...
export default function CronPanel() {
  const { t } = useI18n();
  const { data, isLoading, isFetching, isError, refetch } = useListCronJobsQuery(undefined, {
  ...
  const mutationIsLoading = pendingOp || addPending;
  const busy = isLoading || mutationIsLoading;
  ...
  const handleStartAll = async () => {
    ...
    try {
      ...
      setGatewayError(e?.data?.error || t('cron.failedStartGateways'));
    }
  };
  ...
  const handleStopAll = async () => {
    ...
    try {
      ...
      const e = err as { data?: { error?: string } };
      setGatewayError(e?.data?.error || t('cron.failedStopGateways'));
    }
  };
  ...
  const gatewayUserPending = addPending || pendingOp || starting || stopping;
  const gatewayPending = (gatewayFetching && !!gateway) || gatewayUserPending;
    ...
    <Box sx={{ p: 3, maxWidth: 800, mx: 'auto', width: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        ...
        {!busy && (
          <Typography variant="body2" color="text.secondary">
            {t('cron.jobCount', { count: jobs.length, suffix: jobs.length !== 1 ? 's' : '' })}
          </Typography>
        )}
      ...
      */}
      {(gatewayInitialLoading ||
        (gatewayUserPending && !missing.length && !showRunningPill)) && (
        <Stack
          direction="row"
          alignItems="center"
          spacing={1.25}
        ...
        <Box sx={{ position: 'relative' }}>
          <Alert
            ...
            action={
              ...
              >
                {starting ? <CircularProgress size={14} /> : t('cron.startGateway')}
              </Button>
            }
          >
            <AlertTitle sx={{ fontSize: '0.85rem', fontWeight: 700, mb: 0.25 }}>
              {t('cron.gatewayMissingTitle', { profiles: missing.join(', ') })}
            </AlertTitle>
            <Typography sx={{ fontSize: '0.78rem' }}>
              {t('cron.gatewayMissingDetail')}{' '}
              <em>{t('cron.startGateway')}</em>
            </Typography>
          </Alert>
          {gatewayPending && (
            <CircularProgress
      ...
      {showRunningPill && (
        ...
        >
          ...
          <Typography sx={{ fontSize: '0.78rem', flex: 1 }}>
            {gatewayPending
              ? t('cron.gatewayUpdating')
              : t('cron.gatewayRunning', { profiles: live.join(', ') })}
          </Typography>
          <Chip
            label={t('cron.stopAll')}
            size="small"
            variant="outlined"
            disabled={gatewayBusy}
