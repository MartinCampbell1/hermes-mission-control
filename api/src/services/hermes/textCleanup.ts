...
const SERVICE_TUPLE_PREFIX_RE =
  /^\s*(?:generic:\s*)?\(\s*\d+\s*user\s+messages?\s*,\s*\d+\s*total\s+messages?\s*\)\s*/i;
const TITLED_SERVICE_TUPLE_PREFIX_RE =
  /^\s*(?:generic:\s*)?"[^"]*"\s*\(\s*\d+\s*user\s+messages?\s*,\s*\d+\s*total\s+messages?\s*\)\s*/i;
const RAW_REASONING_WRAPPER_RE = /(?:┌|╭|─)\s*Reasoning\b/i;
const RAW_REASONING_BLOCK_RE = /[┌╭][─━\s]*Reasoning\b([\s\S]*?)(?:[└╰][─━\s]*\n?|$)/gi;
...
export function cleanAssistantMessageText(text: string): string {
  if (!text) return text;
  return text
    .replace(RESUME_BANNER_RE, '')
    .replace(TITLED_SERVICE_TUPLE_PREFIX_RE, '')
    .replace(SERVICE_TUPLE_PREFIX_RE, '')
    .trim();
}

export function hasRawReasoningWrapper(text: string): boolean {
  return RAW_REASONING_WRAPPER_RE.test(String(text || ''));
}

export function cleanReasoningWrapperText(text: string): string {
  if (!text) return '';
  const extracted: string[] = [];
  String(text).replace(RAW_REASONING_BLOCK_RE, (_match, body: string) => {
