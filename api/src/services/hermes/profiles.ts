 ...
 * so we tokenize on whitespace and skip header/divider rows.
 */
export function parseProfileListForTest(stdout: string): { name: string; model: string | null }[] {
  return stripAnsi(stdout)
    .split('\n')
    .map((raw) => raw.replace(/^[\s│┃]+|[\s│┃]+$/g, ''))
    .filter((line) => line && !/^Profile\s+Model/i.test(line) && !/^[─━┯┰]/.test(line))
    .map((line) => line.replace(/^[◆◇•·*\s]+/, '').trim().split(/\s+/).filter(Boolean))
...
function parseProfileList(stdout: string): { name: string; model: string | null }[] {
  return parseProfileListForTest(stdout);
}
